...
const { count, username, password, repassword, phoneNumber, question, answer } = resetPasswordFormData
...
const messageForRepassword = ref(null)
...
watch(resetPasswordFormData, async () => {
    ...
    if (verifyPassport) {
        ...
        isReadonly.value = result.data.code === 1 ? true : false
        isVerify = result.data.code === 1 ? true : false
    }
    ...
    if (matchPassport) {
        ...
        isAnswerReadonly.value = result.data.code === 1 ? true : false
    }
}, {
    deep: true
})
...
const handleSubmit = async () => {
    ...
    // const passport = Object.values(resetPasswordFormData).every(field => field.state === 1)
    ...
    <TheLayout>
        <template #page-header>
            <GroupPageHeader>
                ...
                <template #page-subtitle1>

                </template>
                <!-- <template #page-subtitle2>
                    这是第二段说明这是第二段说明这是第二段说明这是第二段说明这是第二段说明这是第二段说明这是第二段说明这是第二段说明
                </template> -->
            </GroupPageHeader>
